...
def parse_args():
    ...
    parser.add_argument(
        "--fm_weights", type=Path, default="./pretrained_weights/fmif_weights.pt"
    )
...
if __name__ == "__main__":
    ...
    wt_ds = rocklin_df_to_dataset(pd.concat([single_df.iloc[0:1] for _ in range(bs)]))
    ...
    X, S1, mask, lengths, chain_M, residue_idx, mask_self, chain_encoding_all = (
        featurize(wt_batch, device)
    )

    model = FlowMatchPMPNN(vocab=21, num_letters=21)
    ...
    with torch.no_grad():
        ...
        denoising_model = lambda xt, t: model.decode(
            h_V, h_E, E_idx, xt, mask_attend, mask, chain_M
        )[1]

    noisy_classifier = StabilityPMPNN.init()
    noisy_classifier.load_state_dict(
        torch.load(args.predictor_weights, weights_only=False)
    )
    if use_tag:
    ...
    with torch.no_grad():
        ...
        predictor_log_prob = lambda xt, t: torch.log(
            F.sigmoid(
                noisy_classifier.decode(
                    h_V_c[0:1].repeat(xt.shape[0], 1, 1),
                    h_E_c[0:1].repeat(xt.shape[0], 1, 1, 1),
                    E_idx_c[0:1].repeat(xt.shape[0], 1, 1),
                    xt,
                    mask_attend_c[0:1].repeat(xt.shape[0], 1, 1),
                    mask[0:1].repeat(xt.shape[0], 1),
                    chain_M[0:1].repeat(xt.shape[0], 1),
                )
            )
        )
