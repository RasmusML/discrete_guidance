...
from applications.cifar10 import managers, forward_processes, training, utils
...
def main():
    ...
    parser.add_argument(
        "which_model",
        type=str,
        help="which model to train, either denoising or property",
    )
    ...
    if which_model == "denoising":
        ...
        print(
            f"Denoising model number of parameters: {count_params(denoising_model):.3E}"
        )
    else:
        ...
        print(
            f"Property model number of parameters: {count_params(property_model):.3E}"
        )
