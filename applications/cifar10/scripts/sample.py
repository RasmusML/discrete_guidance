...
from applications.cifar10 import managers, forward_processes, model_utils, utils, models
...
def main():
    ...
    parser.add_argument(
        "--parent_dir",
        type=str,
        default="discrete_guidance/applications/cifar10/",
        help="Path to the parent directory where model checkpoints and outputs are saved",
    )
    parser.add_argument(
        "--save_samples_dir",
        type=str,
        default="discrete_guidance/applications/cifar10/outputs/",
        help="The directory to save the generated samples",
    )
    parser.add_argument(
        "--sampler_name", type=str, default="tau_leaping", help="Which sampler to use"
    )
    parser.add_argument(
        "--num_samples",
        type=int,
        default=5000,
        help="Total number of samples, for guided this is the number of samples per class",
    )
    parser.add_argument(
        "-t", "--temp", type=float, default=0.1, help="Set the temperature value."
    )
    parser.add_argument(
        "-g", "--guided", help="If true, do classifier guidance", action="store_true"
    )
    parser.add_argument(
        "-a",
        "--approx",
        help="If true, do classifier guidance with gradient approximation",
        action="store_true",
    )
    ...
    loaded_state = torch.load(
        Path(eval_cfg.denoising_model_checkpoint_path), map_location=device
    )
    ...
    print(
        f"Number of steps: {eval_cfg.sampler.num_steps}, sampler: {eval_cfg.sampler.name}"
    )
    ...
    if guided:
        ...
        loaded_state = torch.load(
            Path(eval_cfg.property_model_checkpoint_path), map_location=device
        )
    ...
    while True:
        ...
        if not guided:
            sampler_outputs = diffusion_manager.generate(
                num_samples=batch_size, num_intermediate_samples=0, guide_temp=temp
            )
            ...
            for i in range(samples.shape[0]):
                path_to_save = os.path.join(
                    eval_cfg.save_samples_dir, f"{i + counter}.png"
                )
            ...
            if grad_approx:
                ...
                for y in all_classes:
                    ...
                    for i in range(samples.shape[0]):
                        path_to_save = os.path.join(
                            eval_cfg.save_samples_dir, f"{y}-{i + counter}.png"
                        )
                        img = Image.fromarray(utils.imgtrans(samples_uint8[i]))
                        img.save(path_to_save)
            else:
        ...
        counter += batch_size
        print(
            f"{counter} out of {total_num_samples} generated, time: {time.time() - start_time:.2f} seconds"
        )
