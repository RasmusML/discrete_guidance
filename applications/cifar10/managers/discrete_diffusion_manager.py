...
class DiscreteDiffusionManager(BaseDiffusionManager):
    ...
    def diffusion_batch_loss(self, batch_data, intensive_loss=True):
        ...
        # Shape (B, D)
        batch_z_jump_t = self._sample_z_jump_t(
            batch_z_t, batch_t, batch_R_t_z_t_vec, batch_y=None
        )
        ...
        batch_hat_Z_t_z_jump_t = self._get_Z_t_z_t(
            batch_z_jump_t, batch_hat_R_t_z_jump_t_vec
        )
        ...
        sig_norm = torch.sum(
            z_jump_t_mask
            * utils.expsublog(prod_R_t_Q_t, batch_Z_t_z_jump_t, self._eps),
            dim=(1, 2),
        )
    ...
    def _get_Z_t_z_t(self, batch_z_t, batch_R_t_z_t_vec):
        ...
        batch_indices = torch.arange(B, device=self.device)[:, None].expand(B, D)
        dim_indices = torch.arange(D, device=self.device)[None, :].expand(B, D)
        batch_R_t_z_t_z_t = batch_R_t_z_t_vec[batch_indices, dim_indices, batch_z_t]
    ...
    def _get_R_t_z_start_t_vec(self, batch_z_start_t, batch_R_t):
        ...
        batch_R_t_z_start_t_vec = batch_R_t[
            torch.arange(B, device=self.device).repeat_interleave(
                D
            ),  # Index the batch dimension
    ...
    def _predict_hat_R_t_z_start_t_vec(
        ...
        batch_fraction_pq_vec = utils.expsublog(
            batch_p_theta_vec_z_start_t, batch_Q_t_vec_z_start_t, eps=self._eps
        )
        # if padding_mask is not None:
        ...
        batch_hat_R_t_z_start_t_vec = utils.expaddlog(
            batch_R_t_vec_z_start_t, batch_sum_pqq_t, eps=self._eps
        )
    ...
    def _adjust_hat_R_t_z_start_t_vec(
        ...
        if self._debug:
            print(
                f"[adjusted] batch_hat_R_t_z_start_t_vec: {batch_hat_R_t_z_start_t_vec.shape}"
            )
    ...
    def _get_E_theta_vec_z_t(self, batch_z_t, batch_t, train_or_eval, batch_y=None):
        ...
        if self.padding_idx is not None:
            ...
            padding_pos_mask = (
                (batch_z_t == self.padding_idx).unsqueeze(-1).expand((B, D, self.S))
            )
    ...
    def _sample_z_jump_t(self, batch_z_t, batch_t, batch_R_t_z_t_vec, batch_y=None):
        ...
        if batch_y is not None:
            ...
            prob_vec_jump_t_z_t = self._get_guided_prob_vec_jump(
                batch_z_t, batch_t, batch_y, prob_vec_jump_t_z_t
            )
        ...
        q_jump_t_index_z_t = torch.distributions.categorical.Categorical(
            prob_vec_jump_t_z_t
        )
    ...
    def _sample_z_1(self, batch_size, num_dimensions):
        ...
        if initial_dist == "GaussianTargetRateForwardProcess":
            ...
            z_1 = cat.sample((batch_size * num_dimensions,)).view(
    ...
    def tau_leaping(
        ...
        if isinstance(data_shape, int):
            ...
            raise ValueError(
                "Only cfg.data.shape with length 3 (for images) or length 1 (for other data) is supported"
            )
        S = self.S
        ...
        for idx, t_last in enumerate(tqdm.tqdm(ts[0:-1])):
            ...
            poisson_dist = torch.distributions.poisson.Poisson(
                hat_R_t_last_z_t_last_vec * tau
            )
            # s - x^d_t, shape (B, D, S)
            diffs = torch.arange(S, device=self.device).view(
                1, 1, S
            ) - batch_z_t_last.unsqueeze(-1)
            ...
            batch_z_t_last = torch.clamp(
                batch_z_t_last + overall_jump, min=0, max=S - 1
            ).long()
            ...
            if t_last in save_ts:
                ...
                if classifier_guidance and idx >= 0:
                    ...
                    prob_ratio_hist.append(
                        guidance_output["prob_ratio"].detach().cpu().numpy()
                    )
                    prob_ratio_approx_hist.append(
                        guidance_output["prob_ratio"].detach().cpu().numpy()
                    )
                    prob_ratio_approx_sec_hist.append(
                        guidance_output["prob_ratio"].detach().cpu().numpy()
                    )
    ...
    def pc_tau_leaping(
        ...
        def _take_poisson_steps(in_x, in_reverse_rates, in_h):
            ...
            # s - x^d_t, shape (B, D, S)
            diffs = torch.arange(S, device=self.device).view(1, 1, S) - in_x.unsqueeze(
                -1
            )
            # Number of jumps in each dimension of each sample
            # Shape (B, D, S)
        ...
        for idx, t_last in enumerate(tqdm.tqdm(ts[0:-1])):
            ...
            batch_z_t_last = _take_poisson_steps(
                batch_z_t_last, hat_R_t_last_z_t_last_vec, h
            )
            ...
            if t_last <= corrector_entry_time:
                for _ in range(num_corrector_steps):
                    ...
                    forward_rates = self._get_R_t_z_start_t_vec(
                        batch_z_t_last, batch_R_t
                    )
    ...
    def _get_property_model_log_prob(
        ...
        self.model_dict[
            "property"
        ].eval()  # Ensure that the model is in evaluation mode
            ...
            for i in range(len(batch_z_t_chunks)):
                ...
                property_log_probs.append(
                    self.model_dict["property"].log_prob(
                        batch_z_t_chunk, batch_t_chunks[i], batch_y_chunks[i]
                    )
                )
    ...
    def _get_all_jump_transitions(self, batch_z_t):
        ...
        # Shape (B*D*S, D)
        batch_z_jump_t_all = self._transition_to_new_state(
            batch_z_t_all, transition_indices
        )
        return batch_z_jump_t_all

    def _get_guided_reverse_rate(
        self, batch_z_t, batch_t, batch_y, batch_hat_R_t_z_t, grad_approx=False
    ):
        """
        ...
        if not grad_approx:
            ...
            # Shape (B, D, S)
            property_log_prob_ratio = property_log_prob - property_log_prob_z_t.view(
                B, 1, 1
            )
            ...
            if not self.categorical:
                ...
                all_diffs = (
                    torch.arange(0, S, device=batch_z_t.device)
                    .unsqueeze(0)
                    .unsqueeze(1)
                    .expand(B, D, S)
                )
                ...
                # Shape (B, D, S)
                property_log_prob_ratio = (
                    all_diffs - all_singles
                ) * grad_log_prob_z_t.unsqueeze(-1)
                ...
                # 1st order Taylor approximation of the log difference
                # Shape (B, D, S)
                property_log_prob_ratio = grad_log_prob_z_t - (
                    batch_z_t * grad_log_prob_z_t
