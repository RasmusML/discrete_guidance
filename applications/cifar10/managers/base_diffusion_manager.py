...
class BaseDiffusionManager(object):
    ...
    """
    def __init__(
            self, cfg, forward_process,
            denoising_model=None, denoising_optimizer=None,
            property_model=None, property_optimizer=None,
        ...
        self.model_dict = {
            'denoising': None if (denoising_model is None) else denoising_model.to(self.device),
            'property':  None if (property_model is None) else property_model.to(self.device),
        }
        self.optim_dict = {
    ...
    def train_on_batch(self, batch_data, which_model, training_state, loader_batch_size=None):
        """
        ...
        if self.clip_grad:
            torch.nn.utils.clip_grad_norm_(self.model_dict[which_model].parameters(), 1.0)

        if self.warmup > 0:
            for g in self.optim_dict[which_model].param_groups:
                g['lr'] = self.lr * np.minimum(training_state['n_iter'] / self.warmup, 1.0)
    ...
    def eval_loss_on_batch(self, batch_data, which_model, loader_batch_size=None, return_logits=False):
        """
        ...
        if which_model=='denoising':
            ...
            batch_output = self.property_batch_loss(batch_data, intensive_loss=True, return_logits=return_logits)
        else:
            err_msg = f"The batch loss is only defined for 'which_model' corresponding to 'denoising' or 'property', got 'which_model={which_model}' instead."
            raise ValueError(err_msg)
        batch_loss = batch_output['loss']
    ...
    def property_batch_loss(self, batch_data, intensive_loss=True, return_logits=False):
        ...
        property_model_output = self._get_property_model_log_prob(batch_z_t, batch_t, batch_y, return_logits=return_logits)
