...
def train_manager_on_cifar(
    ...
    print(
        f"Training the '{which_model}' model for '{num_epochs}' epochs on device '{diffusion_manager.device}'."
    )
    ...
    if which_model != "denoising":
        ...
        accuracy_metric = torchmetrics.classification.MulticlassAccuracy(
            num_classes=cfg.model.num_classes, average=None
        ).to(device)
...
def denoising_images(diffusion_manager, batch):
    ...
    for img_idx in range(3):
        for t_idx in range(len(ts)):
            qt0 = diffusion_manager.forward_process.get_Q_t(
                torch.tensor([ts[t_idx]], device=diffusion_manager.device)
            )  # (B, S, S)
            qt0_rows = qt0[0, x[img_idx, :].flatten().long(), :]
            x_t_cat = torch.distributions.categorical.Categorical(qt0_rows)
            x_t = x_t_cat.sample().view(1, C * H * W)

            x_0_logits = diffusion_manager.model_dict["denoising"](
                {"x": x_t}, torch.tensor([ts[t_idx]], device=diffusion_manager.device)
            ).view(B, C, H, W, S)
            x_0_max_logits = torch.max(x_0_logits, dim=4)[1]

            ax[2 * img_idx, t_idx].imshow(
                imgtrans(x_t.view(B, C, H, W)[0, ...].detach().cpu())
            )
            ax[2 * img_idx, t_idx].axis("off")
            ax[2 * img_idx + 1, t_idx].imshow(
                imgtrans(x_0_max_logits[0, ...].detach().cpu())
            )
...
def train_epochs(
    model, cfg, train_loader, val_loader, writer=None, checkpoint_dir=None
):
    """
    ...
    if "n_epochs" in cfg.training:
        ...
        if "n_iters" not in cfg.training:
            raise ValueError(
                "Either n_epochs or n_iters should be specified for in config.training"
            )
    ...
    state = {"train_losses": [], "val_outputs": dict(), "best_epoch": 0}
    ...
    for epoch in range(num_epochs):
        ...
        for metric in ckpt_metrics:
            ...
            if save_ckpt and checkpoint_dir:
                ...
                print(
                    f"New best val {metric} at epoch {epoch}: {batch_val_output[metric]:.4f}, saved checkpoint"
                )
